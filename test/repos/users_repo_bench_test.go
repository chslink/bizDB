...
import (
	...
	bizdb "github.com/chslink/bizdb"
	"github.com/chslink/bizdb/test/models"
	"github.com/chslink/bizdb/test/repos"
)

// 初始化测试数据
func setupBenchmark(b *testing.B) (*bizdb.MemoryDB, *repos.UserRepo) {
...
func BenchmarkQueryUsers(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = repo.Query(repos.UserQuery{
			Age:   ptr[int](25),
			Limit: ptr[int](100),
		})
	}
...
func BenchmarkConcurrentOperations(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			// 写入
			_ = repo.WithTx(tx).Update(&models.User{
				ID:    1,
			...
			// 查询
			_, _ = repo.WithTx(tx).Query(repos.UserQuery{Age: ptr[int](30)})
...
func BenchmarkTransaction(b *testing.B) {
	...
	b.Run("MultipleOperations", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			tx := db.Begin()
			_ = repo.WithTx(tx).Create(&models.User{
				ID:        int64(1000 + i + 1),
				Name:      "New User",
			...
			_, _ = repo.WithTx(tx).Query(repos.UserQuery{})
			_ = tx.Commit()
		}
	})
}
